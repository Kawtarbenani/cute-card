    ...
    <style>
        * {
        ...
        .non-btn {
            ...
            order: 2;
            margin-top: 10px;
            /* Prevent button from going too far horizontally */
            max-width: 90vw;
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Make Non button move when mouse approaches
            document.addEventListener('mousemove', function(event) {
                ...
                if (distance < 100) {
                    ...
                    // KEEP BUTTON WITHIN VISIBLE VIEWPORT - PREVENT HORIZONTAL SCROLLING
                    const padding = 40;
                    const maxX = window.innerWidth - nonBtn.offsetWidth - padding;
                    const minX = padding;
                    
                    // Constrain horizontal movement more tightly
                    newX = Math.max(minX + 50, Math.min(newX, maxX - 50));
                    newY = Math.max(padding, Math.min(newY, window.innerHeight - nonBtn.offsetHeight - padding));
                    
                    // Smooth movement
                    nonBtn.style.transition = 'left 0.1s linear, top 0.1s linear';
